...
def get_blob_list(container_name: str) -> list:
    """Retrieve a list of blobs in the specified Azure container."""
    blob_service_client = BlobServiceClient.from_connection_string(AZURE_CONNECTION_STRING)
    container_client = blob_service_client.get_container_client(container_name)
    
    print(f"Retrieving blob list from container {container_name}...")
    blob_list = [blob.name for blob in container_client.list_blobs()]
    if not blob_list:
        print(f"No blobs found in container {container_name}.")
        return []
    print(f"Success: Retrieved {len(blob_list)} blobs from Azure container {container_name}.")
